...
if nav == "Prediction":
    ...
    st.title("Prediction")
    next_days = pd.date_range(gp1.index[-1], periods=20, freq='2M')
    ...
    st.write("Original vs Predicted Gold Price")
    fig, ax = plt.subplots(figsize=(15, 7))

    # Plot the gold price and predicted gold price
    ax.plot(m_gp.index, m_gp['price'], label='Gold Price')
    ax.plot(denormalized_prices.index, denormalized_prices['forecasted price'], label='Predicted Gold Price')

    # Set labels and title
    ax.set_xlabel('Date')
    ax.set_ylabel('Price')
    ax.set_title('Gold Price vs Predicted Gold Price')
